...
export function GroupTable({ onEdit }: GroupTableProps) {
  ...
  const listQuery = useQuery({
    queryKey: ["groups", page, DEFAULT_PAGE_SIZE, search],
    queryFn: () =>
      groupsService.list({ page, limit: DEFAULT_PAGE_SIZE, q: search }),
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: Error) => {
      toast.error("ลบกลุ่มไม่สำเร็จ", err.message);
    },
  });
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[var(--line)] bg-[var(--surface)] shadow-[var(--shadow-panel)]">
      ...
      {items.length === 0 ? (
        <EmptyState title="ไม่มีกลุ่ม" description="กดเพิ่มกลุ่มเพื่อเริ่มใช้งาน" />
      ) : (
        <>
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              <thead className="bg-[var(--surface-raised)] text-[11px] uppercase tracking-[0.08em] text-[var(--ink-muted)]">
                <tr>
                  <th className="px-5 py-3 font-semibold">ชื่อกลุ่ม</th>
      ...
      <ConfirmModal
        ...
        description={
          deleteTarget
            ? `ต้องการลบกลุ่ม “${deleteTarget.groupName}” หรือไม่? สมาชิกในกลุ่มจะถูกถอดกลุ่มอัตโนมัติ`
            : ""
        }
